...
| Agent integrations (`common/agent_skills`, agent setup) | Installed skills, reconciliation, readiness checks, MCP registration, generated instructions | Retain skill IDs and tool registration IDs. Existing commands continue via `infra-tools`; update content separately before alias retirement. | Agent skill and readiness tests. Maintainer owns v3 retirement audit. |
